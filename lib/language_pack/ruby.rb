...
See http://devcenter.heroku.com/articles/ruby-versions#selecting_a_version_of_ruby for more information.
WARNING
      end
    end
  ...
  #   Example: http://s3.amazonaws.com/language-pack-ruby/node-0.4.7.tgz, where name is "node-0.4.7"
  ...
  # http://github.com/heroku/heroku-buildpack-ruby/issues/21
...
http://devcenter.heroku.com/articles/sqlite3
ERROR
          end
...
<% end %>
  ...
  def load_bundler_cache
    instrument "ruby.load_bundler_cache" do
      ...
      # fix for http://github.com/heroku/heroku-buildpack-ruby/issues/86
      ...
      # fix for http://github.com/sparklemotion/nokogiri/issues/923
      if @metadata.exists?(buildpack_version_cache) && @metadata.read(buildpack_version_cache).sub('v', '').to_i <= 76
        ...
        puts "See http://github.com/sparklemotion/nokogiri/issues/923."
        purge_bundler_cache
      end
